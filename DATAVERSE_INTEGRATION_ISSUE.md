...
このドキュメントを https://github.com/geekfujiwara/CodeAppsDevelopmentStandard リポジトリのIssueとして登録する場合のテンプレート:

**タイトル:**
```
[提案] Dataverse統合のベストプラクティス追加
```

**本文:**
```markdown
## 概要
Power Apps Code AppsでDataverseテーブルをデータソースとして追加する際のベストプラクティスを開発標準に追加したい。

## 背景
実際のプロジェクトで発見した以下の重要な知見:
- `shared_commondataserviceforapps`ではなく`dataverse`を使用すべき
- Power Apps SDKの使用によりCSP制約を回避できる
- テーブル論理名のみでスキーマが自動生成される

## 提案内容
新セクション「Dataverseテーブル統合ガイド」を追加し、以下を含める:

1. **正しいコマンド**: `pac code add-data-source -a dataverse -t <論理名>`
2. **Power Apps SDK の使用方法**（`@microsoft/power-apps/data`）
3. **避けるべき方法**（`shared_commondataserviceforapps`、直接fetch）
4. **トラブルシューティング**（CDPTabular1エラー、CSPエラー等）
5. **完全なサンプルコード**（Model/Service/UI）

## 詳細ドキュメント
[DATAVERSE_INTEGRATION_ISSUE.md](リンク)を参照
